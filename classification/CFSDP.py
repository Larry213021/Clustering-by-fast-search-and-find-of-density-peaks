...
def show_result(new_class, norm_data, chose_list):
    ...
    for node, class_ in enumerate(new_class):
        ...
        if node in chose_list:
            plt.scatter(x=norm_data[node, 0], y=norm_data[node, 1], marker='*', s=800, c='purple',alpha=1)
        else:
...
def main(input_x):
    a =int(input('input start clusters num 開始分群數: '))
    b =int(input('input final clusters num 最終分群數:　'))
    num_list = []
    list = []
...
if __name__ == '__main__':
    data = r'C:\Users\Larrywu\Desktop\schoolHW\CFSDP\Clustering-Python-master\Aggregation(t=2).txt'
