...
def node_detal(density, distance):
    ...
    for index, node in enumerate(distance):
        ...
        while Stop != True:
            ...
            if step == 1:
                density_larger_than_node = density_larger_than_node[density_larger_than_node != index]
            #  存在密度大于自己的点
        ...
        print('closest_leader','[',index,']: ', closest_leader[index])
    # print('closest_leader','[',121,']: ', closest_leader[121])
    # print('closest_leader', '[', 176, ']: ', closest_leader[176])
    # print('closest_leader', '[', 685, ']: ', closest_leader[685])
    # print('closest_leader', '[', 712, ']: ', closest_leader[712])
...
def clustering(closest_leader, chose_list):
    for i in range(len(closest_leader)):
        ...
        while closest_leader[i] not in chose_list:
            ...
            # time.sleep(1)
            # if (closest_leader[i] == j):
            #     break
            if step > len(closest_leader):
                closest_leader[i] = chose_list[1]
                # closest_leader[i] = np.random.choice(chose_list)
            step += 1
...
def main(input_x):
    # a =int(input('input start clusters num分群數: '))
    # b =int(input('input final clusters num最終分群數:　'))
    a = 6
    b = 6
    num_list = []
    list = []
...
if __name__ == '__main__':
    data = r'C:\Users\Larrywu\Desktop\schoolHW\CFSDP\Clustering-Python-master\Compound(k=5,t=9).txt'
